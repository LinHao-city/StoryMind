...
StoryMind pull requests. It is a review framework, not a closed checklist.
Use it to structure the review, then keep looking for issues that are specific
to the PR in front of you.

StoryMind is an agent-orchestrated video production system. Many PRs affect
...
- Does this PR move StoryMind in the right direction?
...
Check whether the PR solves a real StoryMind problem.
...
StoryMind works with video, audio, image generation, and local models. Small
...
## StoryMind Architecture Checks

### Agent-First Architecture

StoryMind's control plane is the agent following markdown skills and YAML
...
StoryMind uses checkpoints for resume, human approval, audit trails, and
stage gating.
...
StoryMind can compose with Remotion, HyperFrames, or FFmpeg. Runtime choices
...
StoryMind should not silently spend money or imply a paid provider is free.
...
- Is this provider useful for StoryMind workflows?
...
- Is the provider internationally known, nationally or regionally important,
  or otherwise clearly valuable to StoryMind users?
...
Provider viability matters. StoryMind should not become a grab bag of
