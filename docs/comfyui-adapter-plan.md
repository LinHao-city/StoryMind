# ComfyUI Provider Adapter for StoryMind
...
StoryMind's local GPU tools (`wan_video`, `hunyuan_video`, `cogvideo_video`,
...
A ComfyUI adapter gives StoryMind access to any model ComfyUI supports,
...
**Future path:** ACE-Step support should be revisited once StoryMind decides
...
URL when StoryMind knows the canonical source. Agents should surface that
...
using StoryMind's 7-dimension scoring:
...
StoryMind should treat those as custom workflow profiles until a blessed
...
### Future (add models to ComfyUI, no code changes to StoryMind)
...
4. **Music generation:** ACE-Step works in ComfyUI but StoryMind needs a
