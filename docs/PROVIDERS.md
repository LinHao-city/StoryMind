# StoryMind Provider Guide

Everything you need to know about every provider in StoryMind — setup instructions, pricing, free tiers, and what each unlocks.
...
StoryMind now uses those published rates in the Grok tool estimators.
...
StoryMind uses the new-console API key flow:
...
Doubao Speech 2.0 is billed by character package or usage in Volcengine. StoryMind estimates cost from text length and prefers provider-returned usage metadata when available.
...
The CLI is consumed as `npx hyperframes`. Do not use `npx @hyperframes/cli`; that package name is not the StoryMind runtime path.
...
HyperFrames workspaces live under `projects/<project-name>/hyperframes/`. Final videos still follow the normal StoryMind convention: `projects/<project-name>/renders/final.mp4`.
